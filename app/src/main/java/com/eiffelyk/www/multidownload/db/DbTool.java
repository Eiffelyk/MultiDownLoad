package com.eiffelyk.www.multidownload.db;
...
public class DbTool {
    ...
    public synchronized void saveFileState(FileState fileState) {
        SQLiteDatabase database = dbHelper.getWritableDatabase();
        try {
            String sql = "insert into local_info(name,url, state,completeSize,fileSize,image) values (?,?,?,?,?)";
            Object[] bindArgs = {fileState.getName(), fileState.getUrl(),
                    fileState.getState(), fileState.getCompleteSize(),
                    fileState.getFileSize(), fileState.getImage()};
            database.execSQL(sql, bindArgs);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public synchronized List<FileState> getFileStates() {
        ...
        Cursor cursor = null;
        try {
            String sql = "select name,url, state,completeSize,fileSize,image from local_info";
            cursor = database.rawQuery(sql, null);
            while (cursor.moveToNext()) {
                FileState fileState = new FileState(cursor.getString(0), cursor.getString(1), cursor.getInt(2), cursor.getInt(3), cursor.getInt(4),cursor.getString(5));
                list.add(fileState);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public synchronized FileState query(String url) {
        ...
        try {
            ...
            cursor = database.rawQuery(sql, null);
            while (cursor.moveToNext()) {
                 fileState = new FileState(cursor.getString(0), cursor.getString(1), cursor.getInt(2), cursor.getInt(3), cursor.getInt(4), cursor.getString(5));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
